...
export function emit(rootDecl: TopLevelDeclaration, { rootFlags = ContextFlags.None, tripleSlashDirectives = [] }: EmitOptions = {}): string {
    ...
    function printObjectTypeMembers(members: ObjectTypeMember[]) {
        ...
        function printMember(member: ObjectTypeMember) {
            ...
            throw new Error(`Unknown member kind ${(member as ObjectTypeMember).kind}`);
        }
    }
    ...
    function writeTripleSlashDirective(t: TripleSlashDirective) {
        ...
        switch (t.kind) {
            ...
            case "triple-slash-amd-module":
                ...
                break;
            default:
                throw new Error(`Unknown triple slash directive kind ${(t as TripleSlashDirective).kind}`);
        }
        ...
        if (typeof d === 'string') {
            ...
            switch (d.kind) {
                ...
                case "enum":
                    return writeEnum(d);
                default:
                    throw new Error(`Unknown declaration kind ${(d as TopLevelDeclaration).kind}`);
            }
        }
    }
}
